...
//Para el aleteo
float amplitudAleteo;
//float offsetAleteo = 1.0f;
float anguloAleteo;
...
int main()
{
	...
	//Modelo de dragones exportar
	Cuerpo_M = Model();
	Cuerpo_M.LoadModel("Models/Cuerpo.dae");
	AI_M = Model();
	AI_M.LoadModel("Models/AlaIzquierda.dae");
	AD_M = Model();
	AD_M.LoadModel("Models/AlaDerecha.dae");
	...
	//para el dragon offset
	movOffsetDragon = 0.3f;

	//Para el dragón y el ejeAleteo
	glm::vec3 ejeAleteo = glm::vec3(1.0f, 0.0f, 0.0f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Aleteo
		amplitudAleteo = glm::radians(20.0f);
		anguloAleteo = sin(glm::radians(angulovaria * 3.5f)) * amplitudAleteo;

		//---------Ejercicio 9---------------
		tiempAcumulado += deltaTimeAux;
		if (tiempAcumulado >= tiempoEspera)
		{
			mostrarnum1 = !mostrarnum1;
			tiempAcumulado = 0.0f;
		}
		...
		*/
		...
		if (avanzaDragon)
		{
			if (dragonavance > -19.5f) //Se detiene a 20 unidades en el eje -x
			{
		...
		{
			if (dragonavance < 21.5f) //Se detiene a 20 unidades en el eje x
			{
				dragonavance += movOffsetDragon * deltaTime;
			}
			else
		...
		model = glm::rotate(model, -90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		//Aleteo
		model = glm::rotate(model, anguloAleteo, ejeAleteo);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		AI_M.RenderModel();

		//Ala Derecha
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.001f, 0.0f)); //0.001
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		//Aleteo
		model = glm::rotate(model, -anguloAleteo, ejeAleteo);
		...
		*/
		toffsetnumerocambiau += 0.002; // Solo se baja el número de offset de cada número 
		if (toffsetnumerocambiau > 1.0)
			toffsetnumerocambiau = 0.0;
		toffsetnumerov = 0.0;
		toffset = glm::vec2(toffsetnumerocambiau, toffsetnumerov);
		...
		//logica para el cambio de número
		if (mostrarnum1)
		{
			Numero1Texture.UseTexture();
		}
